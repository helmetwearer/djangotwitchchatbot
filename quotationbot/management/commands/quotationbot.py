...
class Command(BaseCommand, IRCMixin):
    ...
    def send_random_quotation_to_json_channel(self, channel, bucket_name):
    ...
    def send_random_quotation_to_twitch_channel(self, channel, bucket_name):
        quotations = Quotation.objects.filter(bucket_name=bucket_name, approved=True)
        count = quotations.count()
        if count and channel.is_bucket_enabled(bucket_name):
            q = quotations[random.randrange(0,count)]
    ...
    def broadcast_available_messages(self, bucket_name):
        ...
        for channel in twitch_channels:
            ...
            if channel.is_live:
                ...
                if channel.is_bucket_enabled(bucket_name):
                    self.send_random_quotation_to_twitch_channel(channel, bucket_name)
        ...
        self.verbose_write('Number of imessage json channels to write to %s' % imessage_channels.count())
        for channel in imessage_channels:
            self.verbose_write('checking channel %s for wake time' % channel.name)
            if channel.is_live:
                self.verbose_write('%s came back as awake, check if bucket is on' % channel.name)
                if channel.is_bucket_enabled(bucket_name):
                    self.send_random_quotation_to_json_channel(channel, bucket_name)
                    channel.available_to_message_after = timezone.now() + timezone.timedelta(
                        minutes=random.randrange(channel.bot_minimum_minutes, channel.bot_maximum_minutes + 1))
                    channel.save()
                    self.verbose_write('%s set to run after %s' % (channel.name, channel.available_to_message_after))
                ...
                self.verbose_write('channel is sleeping, messages will start upon waking hour')
